...
class ZtfRandomForestPredictor(Predictor):
    ...
    def _predict(self, model_input: DataFrame):
        model_result = self.model.predict_in_pipeline(model_input)
        return model_result

    def can_predict(self, model_input: DataFrame):
        return model_input.any().any()
