...
def add_metadata(All_SNID_h5, all_oids, path_md_chunks, dict_cols, dict_info, path_save_dataset, all_partitions, num_folds):
    ...
    if dict_info['type_windows'] == 'windows':
        ...
        df_metadata = pd.merge(df_metadata, 
                               df_aux, 
                               left_on='oid', 
                               right_on='oid_prefix', 
                               how='outer').dropna()
        df_metadata = df_metadata.rename(columns={'oid_y': 'oid'}).drop(['oid_x', 'oid_prefix'], axis=1)[md_cols]
    ...
    for fold in range(num_folds):
        ...
        aux_idx   = aux_pd[aux_pd['partition'] == 'training_{}'.format(fold)].index.to_numpy()
    ...
    add_cols_h5py(df_metadata, 
                  path_save_dataset, 
                  name_dataset='metadata_feat')

    return list(df_metadata.columns)

def add_features(All_SNID_h5, all_oids, path_md_feat_file, dict_cols, dict_info, path_save_dataset, list_times_to_eval, all_partitions, num_folds):

    for time_to_eval in list_times_to_eval:
        print('- adding features {} days'.format(time_to_eval))
        path_feat_chunks = glob.glob('{}/features/{}_days/*'.format(path_md_feat_file, time_to_eval))
        ...
        if dict_info['type_windows'] == 'windows':
            ...
            df_feat = pd.merge(df_feat, 
                               ...
                               right_on='oid_prefix', 
                               how='outer').dropna()
            df_feat = df_feat.rename(columns={'oid_y': 'oid'}).drop(['oid_x', 'oid_prefix'], axis=1)[feat_cols]
        ...
        for fold in range(num_folds):
            ...
            os.makedirs('{}/quantiles/features/{}_days'.format(path_save_dataset, time_to_eval), exist_ok=True)
            dump(qt, '{}/quantiles/features/{}_days/{}.joblib'.format(path_save_dataset, time_to_eval, name_used))

        add_cols_h5py(df_feat, 
                      path_save_dataset, 
...
def add_metadata_QT_as_MLP(All_SNID_h5, all_oids, path_md_chunks, dict_cols, dict_info, path_save_dataset, all_partitions, num_folds):
...
def add_features_QT_as_MLP(All_SNID_h5, all_oids, path_md_feat_file, dict_cols, dict_info, dict_snid_windows, path_save_dataset, list_times_to_eval, all_partitions, num_folds):
