...
def pad_list(lc, nepochs, dict_info, aux_times, flag_detections):
    if dict_info['type_windows'] == 'windows':
        ...
        if pad_num >= 0:
            ...
            return np.array(lc)[np.linspace(0, nepochs - 1, num = dict_info['max_obs']).astype(int)]
    
    elif dict_info['type_windows'] == 'logspace_idx':
        indices = np.logspace(start=np.log10(1), stop=np.log10(nepochs), num=dict_info['max_obs'], endpoint=True, base=10) - 1
        ...
        if pad_num >= 0:
            lc_padded = np.pad(lc_filtered, (0, pad_num), 'constant', constant_values=(0, 0))
        return lc_padded
    
    elif dict_info['type_windows'] == 'logspace_times':
        grid_mjds = np.geomspace(1, MAX_DAYS, num=dict_info['max_obs']) + np.min(aux_times)
        ...
        if len(mjds_before_det) > num_obs_before_det:
            ...
            linspace_mjd_idxs = np.linspace(0, length_forced-1, num=num_obs_before_det).astype(int)
        else:
        ...
        grid_mjds = np.geomspace(0.5, MAX_DAYS, num=dict_info['max_obs']-linspace_mjd_idxs.shape[0]) + np.min(mjds_after_det)
...
def normalizing_time(time_fid):
    mask_min =  9999999999 * (time_fid == 0).astype(float) # Enmascara el PAD generado en separate_by_filter
    t_min = np.min(time_fid + mask_min)
    return (time_fid - t_min) * (~(time_fid == 0)).astype(float) # Considero solo los tiempos donde hay observaciones
...
def get_windows(df_chunk_filtered, dict_cols, dict_info):
    list_df_windows = \
        df_chunk_filtered.groupby(dict_cols['oid']).apply(lambda x: create_windows(x, dict_info['max_obs']))
...
def processing_lc(path_lcs_file, dict_cols, dict_info, df_objid_label):
    print('Generate different windows for each light curve? {}'.format(dict_info['type_windows']))
    ...
    for i, path_chunk in enumerate(path_lcs_chunks):
        ...
        if dict_info['type_windows'] == 'windows':
            ...
            df_chunk_filtered['window_id'] = df_chunk_filtered.groupby(dict_cols['oid']).cumcount() // 200
            df_grouped = df_chunk_filtered.groupby([dict_cols['oid'], "window_id"]).agg(lambda x: list(x)) 

        else:
            df_grouped = df_chunk_filtered.groupby([dict_cols['oid']]).agg(lambda x: list(x))

        pd_dataset = pd.DataFrame()
        for col_name in dict_cols.keys():
            if col_name not in ['oid', 'band', 'class']:
                pd_dataset[col_name] =  \
                    df_grouped.apply(lambda x: separate_by_filter(x[dict_cols[col_name]], 
                                                                  x[dict_cols['band']],  
                                                                  x[dict_cols['time']], 
        ...
        pd_dataset['mask_detection'] = pd_dataset.apply(lambda x: mask_detection(x['mask'], x['detected']), axis=1)
        pd_dataset['time_detection'] = pd_dataset.apply(lambda x: normalizing_time_detection(x['time'], x['mask_detection']), axis=1)

        pd_dataset['mask_photometry'] = pd_dataset.apply(lambda x: mask_photometry(x['mask'], x['detected']), axis=1)
        pd_dataset['time_photometry'] = pd_dataset.apply(lambda x: normalizing_time_photometric(x['time'], x['mask_photometry']), axis=1)
