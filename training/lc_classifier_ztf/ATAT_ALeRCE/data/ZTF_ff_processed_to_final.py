...
def check_files(df_objid_label, dict_cols, dict_info, path_lcs_file, path_md_feat_file):
    ...
    for i, path_chunk in enumerate(path_lcs_chunks):
        ...
        if os.path.exists('./{}/metadata'.format(path_md_feat_file)):
            metadata = pd.read_parquet('{}/metadata/metadata_batch_0{}.parquet'.format(path_md_feat_file, num_batch))
        ...
        if os.path.exists('./{}/features'.format(path_md_feat_file)):
            for time_to_eval in dict_info['list_time_to_eval']:
                features = pd.read_parquet('{}/features/{}_days/feat_batch_0{}.parquet'.format(path_md_feat_file, 
                                                                                              time_to_eval,
                                                                                              num_batch))
...
def main(path_lcs_file, 
    ...
    df_objid_label = df_objid_label[df_objid_label[dict_cols['class']].isin(dict_info['classes_to_use'])].reset_index(drop=True)
    print('Total number of objects after filtering classes used: {}'.format(len(df_objid_label)))

    # We check to have the IDs in all the files that we will use
    all_oids = check_files(df_objid_label, dict_cols, dict_info, path_lcs_file, path_md_feat_file)
    df_objid_label = df_objid_label[df_objid_label[dict_cols['oid']].isin(all_oids)].reset_index(drop=True)

    if dict_info['rm_some_ulens']['rm']:
        ids_rm_ulens = pd.read_csv(dict_info['rm_some_ulens']['path_ulens'])['oid'].values
        df_objid_label = df_objid_label[~df_objid_label[dict_cols['oid']].isin(ids_rm_ulens)].reset_index(drop=True)
    
    all_oids = df_objid_label[dict_cols['oid']].values

    print('Total number of objects used to create the dataset: {}'.format(len(all_oids)))

    #--------------------------------  Generate partitions --------------------------------#
    df_partitions, mapping_to_int = get_partitions(df_objid_label, 
                                                   dict_cols,
                                                   path_save_k_fold)
    
    #--------------------------------  Create dataset.h5 for Light Curves --------------------------------#
    # Revisar el separate_by_filter
    df_lc = processing_lc(path_lcs_file, 
                            dict_cols,
                            dict_info, 
                            df_objid_label)
    ...
    if dict_info['type_windows'] == 'windows':
        ...
        print('We are expanding the number of windows per light curve in test partition...')

        name_cols = [dict_cols['oid'], 'window_id', dict_cols['class'], 'partition', 'label_int']
        partitions_final = []
        ...
        df_partitions[dict_cols['oid']] = df_partitions[dict_cols['oid']].astype(str) + '_' + df_partitions['window_id'].astype(str)
        df_partitions = df_partitions.drop(columns=['window_id'])

        df_lc[dict_cols['oid']] = df_lc[dict_cols['oid']].astype(str) + '_' + df_lc['window_id'].astype(str)
    ...
    if os.path.exists('{}/features'.format(path_md_feat_file)):
        ...
        feat_cols = add_features(All_SNID, 
                                 all_oids,
                                 path_md_feat_file, 
                                 dict_cols, 
                                 dict_info, 
...
if __name__ == "__main__":
    ...
    dict_info = {
        ...
        'rm_some_ulens': {
            'rm': True,
            'path_ulens': '{}/processed/ulens_keep/ts_v9.0.1_b3000_ulensremove.csv'.format(ROOT)},
    ...
    path_save_dataset += '_' + dict_info['type_windows'] + '_' + str(dict_info['max_obs']) + '_'
