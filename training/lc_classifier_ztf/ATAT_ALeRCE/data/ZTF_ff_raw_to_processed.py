...
def extract_info(path_chunk, dict_info):
    ...
    for astro_object in astro_objects_batch:
        ao_features = astro_object['features'][~astro_object['features']['fid'].isin([None])]
        ao_aux_features = astro_object['features'][astro_object['features']['fid'].isin([None])]
    ...
    if dict_info['i'] == 0: 
        ...
        df_metadata.to_parquet('./{}/metadata_batch_{}.parquet'.format(path_save_md, 
                                                                       num_batch))
    ...
    df_features.to_parquet('./{}/feat_batch_{}.parquet'.format(dict_info['path_save_feat_time'], 
                                                               num_batch))
...
def main(dict_info, num_cores, ROOT):
    ...
    for i, time_to_eval in enumerate(dict_info['list_time_to_eval']):
        ...
        path_save_feat_time = '{}/features/{}_days'.format(dict_info['path_save'],
                                                           real_time)
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            _ = list(tqdm.tqdm(pool.imap_unordered(wrapper_extract_info, args_for_pool), 
