...
class PosConcatEmbedding(nn.Module):
    def __init__(self, embedding_size, Tmax=1000.0, input_size=1, trainable=True):
        ...
        initial_div_term = torch.exp(torch.arange(0.0, self.embedding_size, 2).float() * -(math.log(Tmax) / self.embedding_size))
        ...
        self.linear_proj = nn.Sequential(
            nn.Linear(in_features=input_size, out_features=self.embedding_size, bias=True),
        )
