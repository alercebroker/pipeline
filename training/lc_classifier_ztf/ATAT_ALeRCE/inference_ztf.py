...
def get_chunks(data_dict_eval_time, eval_time, batch_size, config_used):
    ...
    data_dict_eval_time['test_{}'.format(eval_time)]['data_err'] = \
        [data_dict_eval_time['test_{}'.format(eval_time)]['data_err'][x:x+batch_size] for x in range(0, len(data_dict_eval_time['test_{}'.format(eval_time)]['data_err']), batch_size)]
    ...
    if config_used['general']['use_metadata']:
        data_dict_eval_time['test_{}'.format(eval_time)]['metadata_feat'] = \
            [data_dict_eval_time['test_{}'.format(eval_time)]['metadata_feat'][x:x+batch_size] for x in range(0, len(data_dict_eval_time['test_{}'.format(eval_time)]['metadata_feat']), batch_size)]

    if config_used['general']['use_features']:
        data_dict_eval_time['test_{}'.format(eval_time)]['extracted_feat'] = \
            [data_dict_eval_time['test_{}'.format(eval_time)]['extracted_feat'][x:x+batch_size] for x in range(0, len(data_dict_eval_time['test_{}'.format(eval_time)]['extracted_feat']), batch_size)]
...
def get_predictions(path_exp, data_root, list_eval_time, batch_size, device, partition_used):
    ...
    if config_used['general']['use_metadata']:
        ...
        data_dict.update({"metadata_feat": torch.from_numpy(metadata_feat).float().unsqueeze(2)})
    ...
    if config_used['general']['use_features']:
        ...
        for time_eval in config_used['general']['list_time_to_eval']:
            ...
            path_QT = "./{}/quantiles/features/{}_days/fold_{}.joblib".format(data_root, last_time, partition_used)
            extracted_feat_aux = h5_file.get("extracted_feat_{}".format(time_eval))[:][these_idx]
            if config_used['general']['use_QT']:
                QT = load(path_QT)
    ...
    for eval_time in list_eval_time:
        ...
        if config_used['general']['use_features']:
            data_dict['extracted_feat'] = torch.from_numpy(extracted_feat[eval_time]).float().unsqueeze(2)
        ...
        data_dict_eval_time = get_chunks(data_dict_eval_time, eval_time, batch_size, config_used)
    ...
    for _, set_eval_time in enumerate(data_dict_eval_time.keys()):
        ...
        for i in range(len(data_dict_eval_time[set_eval_time]['data'])):
            ...
            if config_used['general']['use_lightcurves_err']:
                batch['data_err'] = data_dict_eval_time[set_eval_time]['data_err'][i].to(device)

            tabular_features = []
            if config_used['general']['use_metadata']:
                tabular_features.append(data_dict_eval_time[set_eval_time]['metadata_feat'][i].to(device))

            if config_used['general']['use_features']:
                tabular_features.append(data_dict_eval_time[set_eval_time]['extracted_feat'][i].to(device))

            if tabular_features:
                batch["tabular_feat"] = torch.cat(tabular_features, axis=1)

            preds_lc, preds_tab, preds_mix = model(**batch)
            pred = preds_mix if preds_mix is not None else (preds_lc if preds_lc is not None else preds_tab)
            ...
            preds_prob_out = (
                np.concatenate([preds_prob_out, pred], axis=0)
                if i != 0
                else pred
            )
