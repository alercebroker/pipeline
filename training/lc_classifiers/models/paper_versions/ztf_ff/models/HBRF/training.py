...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HBRF Training Script")

    # Rutas por defecto (puedes cambiarlas aquí o por argumentos)
    name_data_version = "250408_ndetge8"
    default_path_features = f"../../data/preprocessed/data_{name_data_version}_ao_shorten_features"
    default_path_objects = "../../data/preprocessed/objects_250410.parquet"
    default_path_partition = f"../../data/partitions/{name_data_version}/partitions.parquet"

    # Argumentos de rutas y configuración general
    parser.add_argument("--path_features", type=str, default=default_path_features, help="Ruta a las features")
    parser.add_argument("--path_objects", type=str, default=default_path_objects, help="Ruta a los objetos")
    parser.add_argument("--path_partition", type=str, default=default_path_partition, help="Ruta a la particion")
    parser.add_argument("--num_workers", type=int, default=20, help="Numero de workers para carga de datos")
    
    # Flag booleano: si se pone --searching_hyperparameters es True, si no, es False
    parser.add_argument("--searching_hyperparameters", action="store_true", help="Activar búsqueda de hiperparámetros")

    # Hiperparámetros (se reciben como strings que representan listas, ej: "[10, 100]")
    parser.add_argument("--n_trees", type=str, default="[500]", help="Lista de n_trees")
    parser.add_argument("--criterion", type=str, default="['gini']", help="Lista de criterion")
    parser.add_argument("--max_depth", type=str, default="[100]", help="Lista de max_depth (usar None sin comillas si aplica)")
    parser.add_argument("--folds_to_run", type=str, default="[0, 1, 2, 3, 4]", help="Lista de folds a ejecutar")

    args = parser.parse_args()

    # Convertir strings a listas de Python
    try:
        n_trees_list = ast.literal_eval(args.n_trees)
        criterion_list = ast.literal_eval(args.criterion)
        max_depth_list = ast.literal_eval(args.max_depth)
        folds_list = ast.literal_eval(args.folds_to_run)
    except ValueError as e:
        print(f"Error parseando las listas de argumentos: {e}")
        sys.exit(1)
    ...
    # Features a remover hardcoded (o podrías agregarlas a argparse también)
    rm_feat = ['TDE_mag0', 'fleet_t0', 'fleet_m0', 'ulens_t0', 'ulens_mag0']

    print(f"Running with:")
    print(f"  Partition: {args.path_partition}")
    print(f"  Searching HP: {args.searching_hyperparameters}")
    print(f"  Folds: {folds_list}")
    ...
    run_with_hyperparams(
        ...
        rm_feat,
        args.num_workers,
        args.searching_hyperparameters
    )