...
class LitATAT(L.LightningModule):
    def __init__(self, **kwargs):
        ...
        if "float32" in self.general_['data_root']:
            self.atat = self.atat.float()
        ...
        self.entropy_beta = kwargs["general"]["entropy_beta"]
        self.label_smoothing = kwargs["general"]["label_smoothing"]

        ## --- INSPECCIÓN DE MÓDULOS Y PARÁMETROS ---
        #print("\n=== MÓDULOS DEL MODELO ===")
...
#
        ...
        #    print(f"{name:50} | shape: {str(tuple(param.shape)):>20} | requires_grad: {param.requires_grad:<5} | params: {num_params}")
#
#
...
#
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        for y, y_type in zip([pred_lc, pred_tab, pred_mix], ["lc", "tab", "mix"]):
            if y is not None:
                ce_loss = F.cross_entropy(y, y_true, label_smoothing=self.label_smoothing)
                probs = torch.softmax(y, dim=-1)
                entropy_term = torch.sum(probs * torch.log(probs + 1e-8), dim=-1).mean()
                total_loss = ce_loss + self.entropy_beta * entropy_term

                loss += total_loss
                loss_dic.update({
                    f"loss_train/{y_type}_ce": ce_loss,
                    f"loss_train/{y_type}_entropy": entropy_term,
                    f"loss_train/{y_type}": total_loss,
                })
    ...
    def validation_step(self, batch_data, batch_idx):
        ...
        for y, y_type in zip([pred_lc, pred_tab, pred_mix], ["lc", "tab", "mix"]):
            if y is not None:
                ...
                loss += total_loss
                loss_dic.update({
                    f"loss_validation/{y_type}_ce": ce_loss,
                    f"loss_validation/{y_type}_entropy": entropy_term,
                    f"loss_validation/{y_type}": total_loss,
                })
    ...
    def test_step(self, batch_data, batch_idx, dataloader_idx=None):
        if dataloader_idx is None:
            # Si no hay múltiples dataloaders, asumir que hay solo un valor en list_time_to_eval
            time_to_eval = self.list_time_to_eval[0]
        else:
        ...
        for y, y_type in zip([pred_lc, pred_tab, pred_mix], ["lc", "tab", "mix"]):
            if y is not None:
                ...
                loss += total_loss
                loss_dic.update({
                    f"loss_test/{time_to_eval}_days/{y_type}_ce": ce_loss,
                    f"loss_test/{time_to_eval}_days/{y_type}_entropy": entropy_term,
                    f"loss_test/{time_to_eval}_days/{y_type}": total_loss,
                })
