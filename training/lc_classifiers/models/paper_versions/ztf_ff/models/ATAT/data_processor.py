...
def convert_floats(df, dtype):
    return df.astype({col: dtype for col in df.select_dtypes(include='float64').columns})
...
def extract_info(path_chunk, dict_info, dict_cols, dir_astro_features):
    ...
    for i, astro_object in enumerate(astro_objects_batch):
        ...
        #detections = convert_floats(detections, 'float32')
        #forced_photometry = convert_floats(forced_photometry, 'float32')
        ...
        #lightcurve = lightcurve.iloc[0:199]
        ...
        if dict_info['extract_metadata']:
            ...
            #metadata = convert_floats(metadata, 'float32')
        ...
        if dict_info['extract_features']:
            for time_to_eval, astro_objects_batch_feat in dict_astro_objects_feat.items():
                ...
                #features = convert_floats(features, 'float32')
...
if __name__ == "__main__":
    ...
    dir_astro_lightcurves = "{}/preprocessed/data_250408_ndetge8_ao".format(ROOT)
    dir_astro_features = "{}/preprocessed/data_250408_ndetge8_ao_shorten_features".format(ROOT)

    path_partitions = [
        '{}/partitions/250408_ndetge8/partitions.parquet'.format(ROOT),
        '{}/partitions/250408_ndetge8_20folds/partitions.parquet'.format(ROOT),
        '{}/partitions/250408_ndetge8_sanchez_tax_20folds/partitions.parquet'.format(ROOT)
    ]

    use_multiprocessing = True
    num_cores_to_used = 20

    for path_partition in path_partitions:
    
        name_version = path_partition.split('/')[-2]
        timestamp = datetime.now().strftime("%y%m%d")
        dict_info = {
            "path_save": "data/processed/ds_pre{}_pos{}".format(name_version, timestamp),
            "type_windows": "windows", # windows, max, none
            "max_obs": 200,
            "extract_metadata": True,
            "extract_features": True,
            "list_time_to_eval": [8, 16, 32, 64, 128, 256, 512, 1024, None], # [None], 
        ...
        num_cores = min(os.cpu_count()-2, num_cores_to_used)

        logging.info(f"Starting main process using {num_cores} workers")
