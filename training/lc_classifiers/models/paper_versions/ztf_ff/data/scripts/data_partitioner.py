...
def create_partitions(objects, dir_save_partition, num_folds, dict_keep_test=None, SplitObject=None):
    ...
    if dict_keep_test is None:
        _, X_test, _, y_test = train_test_split(X, y, test_size=0.2, stratify=y, random_state=42)
    else:
        name_prev_partition = dict_keep_test['name_prev_partition']
        partitions = pd.read_parquet(f'{name_prev_partition}/partitions.parquet')
        objects = objects[objects.oid.isin(partitions.oid.unique())]
    ...
    if SplitObject is not None:
        kf = SplitObject(n_splits=num_folds, test_size=0.2, random_state=0)
    else:
...
if __name__ == "__main__":
    ...
    path_all_objects = f'{ROOT}/objects_250410.parquet'
    dir_astro_lightcurves = f"{ROOT}/data_250408_ndetge8_ao"
    dir_save_partition = f"data/partitions/250408_ndetge8"
