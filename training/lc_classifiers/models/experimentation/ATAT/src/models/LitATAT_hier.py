...
from src.layers.HierarchicalATAT import HierarchicalATAT
...
class LitATAT(L.LightningModule):
    def __init__(self, **kwargs):
        ...
        self.atat = HierarchicalATAT(**kwargs)
    ...
    def get_input_data(self, batch_data):
        ...
        if self.use_lightcurves:
            input_dict.update(
                {
                    "data": batch_data["brightness"].float(),
        ...
        if self.use_lightcurves_err:
            input_dict.update({"data_err": batch_data["e_brightness"].float()})
