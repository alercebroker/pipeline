...
def extract_info(path_chunk, dict_info, dict_cols, dir_astro_features):
    ...
    batch = pd.read_parquet("./{}".format(path_chunk))
    ...
    df_dataset = []
    for oid, lightcurve in batch.groupby('oid'):
        #lightcurve['detected'] = 1
...
def main(dict_info, dict_cols, num_cores, dir_astro_lightcurves, dir_astro_features, path_partition, use_multiprocessing=True):
    ...
    path_chunks = glob.glob(f"{dir_astro_lightcurves}/*")
    ...
    dict_info.pop('md_col_names', None) 
...
if __name__ == "__main__":
    ROOT = "../../../data_acquisition/atlas"

    dir_astro_lightcurves = "{}/preprocessed/data_250304_limitMJD_woFluxMask_detected".format(ROOT)
    dir_astro_features = None
    path_partition = '{}/preprocessed/partitions/241209_ndetge8/partitions.parquet'.format(ROOT)

    timestamp = datetime.now().strftime("%y%m%d")
    dict_info = {
        "path_save": "data/atlas/processed/ds_pre241209_pos{}_detff_limitMJD_woFluxMask_detected".format(timestamp),
        ...
        "bands_to_use": ['o', 'c'] # NO esta haciendo nada aqui, pero se usa en el entrenamiento, quizas agregarlo al dict_info desde los mismos AO
    }
