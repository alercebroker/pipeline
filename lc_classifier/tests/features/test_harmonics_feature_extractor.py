...
class TestHarmonicsFeatureExtractor(unittest.TestCase):
    def test_ztf_object(self):
        ...
        if plot:
            ...
            r_harmonics = ztf_astro_object.features[ztf_astro_object.features['fid'] == 'r'].copy()
            r_harmonics.set_index('name', inplace=True)
            r_harmonics = r_harmonics['value']
            offset = 0.32
            brightness = r_harmonics.loc['Harmonics_mag_1'] * np.sin(2*np.pi/period_value*(time_grid + offset))
            for i in range(2, 8):
                phase = r_harmonics.loc[f'Harmonics_phase_{i}']
                brightness += r_harmonics.loc[f'Harmonics_mag_{i}'] * np.sin(i * 2 * np.pi / period_value * (time_grid + offset) + phase)
        ...
        brightness = r_harmonics.loc['Harmonics_mag_1'] * np.sin(2 * np.pi / period_value * (time + offset))
        for i in range(2, 8):
            phase = r_harmonics.loc[f'Harmonics_phase_{i}']
