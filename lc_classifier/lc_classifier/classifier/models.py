...
class HierarchicalRandomForest(BaseClassifier):
    ...
    def check_loaded_models(self):
        assert set(self.top_classifier.classes_.tolist()) == set(
            self.taxonomy_dictionary.keys()
        )

        assert set(self.stochastic_classifier.classes_.tolist()) == set(
            self.taxonomy_dictionary["Stochastic"]
        )

        assert set(self.transient_classifier.classes_.tolist()) == set(
            self.taxonomy_dictionary["Transient"]
        )

        assert set(self.periodic_classifier.classes_.tolist()) == set(
            self.taxonomy_dictionary["Periodic"]
        )
    ...
    def download_model(self):
        ...
        if self.url_model[-len(compressed_extension) :] == compressed_extension:
            ...
            wget.download(self.url_model, full_filename)
...
class ElasticcRandomForest(HierarchicalRandomForest):
    ...
    def __init__(
        ...
        if model_name is not None:
            ...
            self.MODEL_PICKLE_PATH = os.path.join(
                PICKLE_PATH, f"{self.MODEL_VERSION_NAME}"
            )
    ...
    def check_missing_features(self, available_features):
        ...
        required_features = set(np.concatenate(list(self.feature_list_dict.values())))
    ...
    def fit(self, samples: pd.DataFrame, labels: pd.DataFrame) -> None:
        ...
        selected_top_snids, _ = rus.fit_resample(
            samples.index.values.reshape(-1, 1), labels["classALeRCE"]
        )
    ...
    def save_model(self, directory: str) -> None:
        ...
        with open(
            os.path.join(directory, self.pickles["feature_list_dict"]), "wb"
        ) as f:
    ...
    def load_model(self, directory: str, n_jobs: int) -> None:
        ...
        with open(
            os.path.join(directory, self.pickles["feature_list_dict"]), "rb"
        ) as f:
...
class ElasticcMLP(BaseClassifier):
    ...
    def fit(
        ...
        training_dataset = self._tf_dataset_from_dataframes(
            x_training, y_training, y_teacher, balanced=True, fit_preprocessor=True
        )
        print("tf dataset validation")
        validation_dataset = self._tf_dataset_from_dataframes(
            x_validation, y_validation, None, balanced=False, fit_preprocessor=False
        )
        print("tf dataset test")
        test_dataset = self._tf_dataset_from_dataframes(
            x_test, y_test, None, balanced=False, fit_preprocessor=False
        )

        training_dataset = training_dataset.shuffle(self.batch_size * 25).batch(
            self.batch_size
        )
        ...
        if y_teacher_df is not None:
            ...
            if balanced:
                ...
                for class_i in range(len(self.list_of_classes)):
                    ...
                    tf_dataset_from_class = (
                        tf_dataset_from_class.repeat()
                        .shuffle(100, reshuffle_each_iteration=True)
                        .prefetch(20)
                    )
                    datasets_from_class.append(tf_dataset_from_class)
                tf_dataset = tf.data.Dataset.sample_from_datasets(datasets_from_class)
            else:
    ...
    def _label_list_to_one_hot(self, label_list: np.ndarray):
        matches = np.stack([label_list == s for s in self.list_of_classes], axis=-1)
    ...
    def predict_proba(self, samples: pd.DataFrame) -> pd.DataFrame:
        ...
        samples = self.feature_preprocessor.preprocess_features(samples).values.astype(
            np.float32
        )
        ...
        probs_df = pd.DataFrame(
            data=probs, columns=self.list_of_classes, index=samples_index
        )
        return probs_df
    ...
    def save_model(self, directory: str) -> None:
        ...
        self.mlp.save_weights(os.path.join(directory, "mlp.h5"))
    ...
    def load_model(self, directory: str) -> None:
        ...
        self.mlp.load_weights(os.path.join(directory, "mlp.h5"))
