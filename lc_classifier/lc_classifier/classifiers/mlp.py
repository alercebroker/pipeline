...
class MLPClassifier(Classifier):
    ...
    def classify_batch(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        dataframe = pd.DataFrame(
            data=probs_np, columns=self.list_of_classes, index=features_df.index
        )
        return dataframe
    ...
    def fit(self, features: pd.DataFrame, labels: pd.DataFrame, config: Dict):
        ...
        self.full_model = MLPModel(
            self.list_of_classes, config["learning_rate"], config["batch_size"]
        )
    ...
    def save_classifier(self, directory: str):
        ...
        self.preprocessor.save(os.path.join(directory, "preprocessor.pkl"))

        with open(os.path.join(directory, "features_and_classes.pkl"), "wb") as f:
            pickle.dump(
                {
        ...
        export_archive.write_out(os.path.join(directory, "tf_model"))

    def load_classifier(self, directory: str):
        self.preprocessor.load(os.path.join(directory, "preprocessor.pkl"))

        features_and_classes = pd.read_pickle(
            os.path.join(directory, "features_and_classes.pkl")
        )
        self.feature_list = features_and_classes["feature_list"]
        self.list_of_classes = features_and_classes["list_of_classes"]

        self.inference_model = tf.saved_model.load(os.path.join(directory, "tf_model"))


class MLPModel(tf.keras.Model):
    def __init__(
        self, list_of_classes: List[str], learning_rate: float, batch_size: int
    ):
        super().__init__()
        self.list_of_classes = list_of_classes
        ...
        self.dense_layer_4 = Dense(len(self.list_of_classes), name="dense_layer_4")
    ...
    @tf.function
    def train_step(self, x_batch: tf.Tensor, y_batch: tf.Tensor):

        with tf.GradientTape() as tape:
            y_pred = self.__call__(x_batch, training=True, logits=True)
            loss_value = self.loss_computer(y_batch, y_pred)

            loss_value = loss_value + tf.keras.regularizers.L1(l1=1e-5)(
                self.dense_layer_1.kernel
            )
    ...
    def validation_or_test_step(
        ...
        for x_batch, y_batch in dataset:
            y_pred = self.__call__(x_batch, training=False, logits=False)
    ...
    def _compute_stats(
        self, labels: np.array, predicted_probabilities: np.array
    ...
    def train(
        ...
        for x_batch, y_batch in training_dataset:
            ...
            if iteration % 2500 == 0:
                ...
                print(
                    f"iteration {iteration} valstats f1 {val_f1:.3f} "
                    f"precision {val_precision:.3f} recall {val_recall:.3f}"
                )

                should_stop = self._evaluate_training_stopper(
                    -val_f1
                )  # minus because it expects a loss
                if should_stop:
                    break

    def _tf_dataset_from_dataframes(
        self, features: pd.DataFrame, labels: pd.DataFrame, rebalance: bool
    ):
        ...
        if rebalance:
            ...
            for class_i in range(len(self.list_of_classes)):
                ...
                tf_dataset_from_class = tf_dataset_from_class.repeat().shuffle(
                    100, reshuffle_each_iteration=True
                )
                datasets_from_class.append(tf_dataset_from_class)
            dataset = tf.data.Dataset.sample_from_datasets(datasets_from_class)
        else:
    ...
    def _evaluate_training_stopper(self, current_validation_loss: float) -> bool:
        ...
        if (
            ...
            >= self.max_validations_without_improvement
        ):
            return True
        else:
            return False
