...
from ..utils import all_features_from_astro_objects
...
class LightGBMClassifier(Classifier):
    ...
    def classify_batch(
        self, astro_objects: List[AstroObject], return_dataframe: bool = False
        ...
        features_df = features_df.rename(
            columns=lambda x: x.replace("_", "").replace(",", "")
        )
        features_df = features_df[self.feature_list]
        features_df = self.preprocessor.preprocess_features(features_df)
        probs_np = self.model.predict(
            features_df, num_iterations=self.model.best_iteration
        )
        for object_probs, astro_object in zip(probs_np, astro_objects):
            data = np.stack([self.list_of_classes, object_probs.flatten()], axis=-1)
            object_probs_df = pd.DataFrame(data=data, columns=[["name", "value"]])
    ...
    def fit_from_features(
        self, features: pd.DataFrame, labels: pd.DataFrame, config: Dict
    ):
        ...
        train_data = lgb.Dataset(
            training_features, label=training_labels["astro_class_num"]
        )
    ...
    def save_classifier(self, directory: str):
        ...
        with open(os.path.join(directory, "lightgbm_model.pkl"), "wb") as f:
            pickle.dump(
                {
