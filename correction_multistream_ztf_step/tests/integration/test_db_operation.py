...
@pytest.mark.usefixtures("psql_service")  # Usando el mismo fixture que el primer script
class TestCorrectionMultistreamZTF(unittest.TestCase):
    ...
    def test_correction_step_execution(self):
        ...
        # Configurar el consumer de prueba para que devuelva nuestros mensajes
        ...
        try:
            # Pre-consume y configuración
            self.step._pre_consume()
            
            # Procesar todos los mensajes en el consumer
            processed_messages = []
            ...
            # Si llegamos aquí sin errores, la prueba básica pasó
            print('processed_messages', len(processed_messages))
            self.assertTrue(len(processed_messages) > 0, "No se procesaron mensajes")
            
            # Verificaciones adicionales si los mensajes se procesaron correctamente
            if hasattr(self.step.producer, "pre_produce_message") and self.step.producer.pre_produce_message:
                ...
                # Si hay suficientes datos, verificar conteos específicos
