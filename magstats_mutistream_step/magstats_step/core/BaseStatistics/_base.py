...
class BaseStatistics(abc.ABC):
    ...
    # Map sid to survey name (ZTF uses sid = 0)
    _SURVEY_DICT = {0: "ZTF", 1: "LSST"}
    ...
    def _survey_mask(self, survey: str) -> pd.Series:
        survey_names = self._detections["sid"].map(self._SURVEY_DICT)
        return survey_names.str.lower() == survey.lower()
