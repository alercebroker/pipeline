...
ALLWISE_KEYS = ['oid_catalog','ra','dec','w1mpro','w2mpro','w3mpro','w4mpro','w1sigmpro','w2sigmpro','w3sigmpro','w4sigmpro','j_m_2mass','h_m_2mass','k_m_2mass','j_msig_2mass','h_msig_2mass','k_msig_2mass']
...
class XmatchStep(GenericStep):
	...
	def cast_allwise(self, wise_match: dict) -> dict:
		return {key: wise_match[key] for key in ALLWISE_KEYS if key in wise_match.keys()}
	...
	def save_xmatch(self, result):
		result = result.rename({'AllWISE': 'oid_catalog', 'RAJ2000': 'ra', 'DEJ2000': 'dec', 'W1mag': 'w1mpro', 'W2mag': 'w2mpro',
						'W3mag': 'w3mpro', 'W4mag': 'w4mpro', 'e_W1mag': 'w1sigmpro', 'e_W2mag': 'w2sigmpro', 'e_W3mag': 'w3sigmpro',
						'e_W4mag': 'w4sigmpro', 'Jmag': 'j_m_2mass', 'Hmag': 'h_m_2mass', 'Kmag': 'k_m_2mass', 'e_Jmag': 'j_msig_2mass', 'e_Hmag': 'h_msig_2mass',
						'e_Kmag': 'k_msig_2mass','oid_in': 'oid', 'angDist': 'dist'}, axis='columns')
		...
		for d in res_arr:
			...
			object, created = self.driver.session.query().get_or_create(Object, filter_by=filter)
			filter = {
				"oid_catalog": d["oid_catalog"]
			}
			data = self.cast_allwise(d)
			allwise, created = self.driver.session.query().get_or_create(Allwise, filter_by=filter, **data)
