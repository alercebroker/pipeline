...
class FakeMetric:
    ...
    def create_fake_metric(self, candid: str = None, source: str = None):
        ...
        selected_candid = candid or self.fake.random.choice([fake_candids, fake_candid])

        return {
            "tid": ["ZTF"] * len(selected_candid)
            if type(selected_candid) == list
            else "ZTF",
