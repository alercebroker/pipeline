...
class PSQLIntegrationTest(unittest.TestCase):
    ...
    def test_insert_db(self):
        features = self.step._format_features(self.message["features"])
        result = self.step.model.predict_in_pipeline(features)
        self.step.driver.session.query().get_or_create(
            Object, {
                "oid": self.message["oid"]
            }
        )

        self.step.insert_db(result, self.message["oid"])
        self.assertEqual(len(self.step.driver.query(Object).all()), 1)
        self.assertEqual(len(self.step.driver.query(Probability).all()), 33)
