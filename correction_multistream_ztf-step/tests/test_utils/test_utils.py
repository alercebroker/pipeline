...
def step_creator():
    settings = config_from_yaml_file("/home/kay/Documentos/alerce/new_step/pipeline/correction_multistream_ztf-step/tests/test_utils/config.yaml")
...
def validate_alert_fields_message(data):  
    num_msg = 0 
    for message in data: 
        required_top_fields = ['oid', 'measurement_id', 'meanra', 'meandec', 'detections', 'non_detections'] # List of columns expected in the outer part of any alert
        missing_top_fields = [field for field in required_top_fields if field not in message]
        
        if missing_top_fields:
            print(f"The following fields are missing at the top level of the message number {num_msg}: {missing_top_fields}")
            return False
    ...
    for message in data: 
        if 'non_detections' in message:
            ...
            if message['non_detections']:  
                required_non_det_fields = ['aid', 'oid', 'sid', 'tid', 'band', 'mjd', 'diffmaglim']
                for i, non_det in enumerate(message['non_detections']):
                    missing_non_det_fields = [field for field in required_non_det_fields if field not in non_det]
                    if missing_non_det_fields:
                        print(f"Non-detection at index {i} is missing fields: {missing_non_det_fields}")
                        return False
                    
                    unexpected_non_det_fields = [field for field in non_det if field not in required_non_det_fields]
                    if unexpected_non_det_fields:
                        print(f"Unexpected fields found in non-detection at index {i} in message number {num_msg}: {unexpected_non_det_fields}")
                        return False
    ...
    for message in data: 
        if 'detections' in message:
            ...
            if message['detections']:  
                required_det_fields = ['aid', 'oid', 'sid', 'tid', 'pid', 'band', 'measurement_id', 'mjd', 'ra', 'e_ra', 'dec', 'e_dec', 'mag', 'e_mag', 'mag_corr', 'e_mag_corr', 'e_mag_corr_ext', 'isdiffpos', 'parent_candid', 'has_stamp', 'corrected', 'dubious', 'stellar', 'forced', 'new', 'extra_fields']
                for i, det in enumerate(message['detections']):
                    missing_det_fields = [field for field in required_det_fields if field not in det]
                    if missing_det_fields:
                        print(f"Detection at index {i} is missing fields: {missing_det_fields}")
                        return False
                    
                    unexpected_det_fields = [field for field in det if field not in required_det_fields]
                    if unexpected_det_fields:
                        print(f"Unexpected fields found in detection at index {i} in message number {num_msg}: {unexpected_det_fields}")
                        return False
...
def message_validation(data):
    return validate_alert_fields_message(data) and validate_non_detection_fields_message(data) and validate_detection_fields_message(data)
...
if __name__ == "__main__":
    ...
    for message in step.consumer.consume():
        ...
        messages_produce = step.producer.pre_produce_message[0] # All messages produced are saved in this 
        ...
        assert output_expected_count(messages_produce, 661678953, expected_dets=3, expected_non_dets=3)
        assert output_expected_count(messages_produce, 879453281, expected_dets=6, expected_non_dets=6)
