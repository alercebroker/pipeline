...
def parse_sql_detection(ztf_models: list, models: list, *, oids) -> list:
    ...
    for det in ztf_models:
        ...
        for field, value in det.items():
            if field.startswith("_"):
                continue
            elif not field in GENERIC_FIELDS_FP:
    ...
    for detections in parsed_dets:
        ...
        for name in CHANGE_NAMES.keys():
            parsed_ztf_list[parsed_dets.index(detections)].__dict__['extra_fields'][CHANGE_NAMES[name]] = parsed_ztf_list[parsed_dets.index(detections)].__dict__['extra_fields'][name]
...
def parse_sql_forced_photometry(ztf_models: list, models: list, *, oids) -> list:
    ...
    for d in ztf_models:
        ...
        for key, value in d[0].__dict__.items():
            if key.startswith("_"):
                continue
            elif not key in GENERIC_FIELDS_FP:
