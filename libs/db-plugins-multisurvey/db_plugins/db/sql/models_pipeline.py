...
class LsstDetection(Base):
    ...
    trailAlgorithm = Column(Integer)
    trail_flag_edge = Column(Boolean)
    trail_flag = Column(Boolean)
    ...
    reliabilityVersion = Column(VARCHAR)
...
class GaiaDR3_source(Base):
    ...
    ra = Column(DOUBLE_PRECISION, nullable=False)
    ra_error = Column(DOUBLE_PRECISION, nullable=False)
    dec = Column(DOUBLE_PRECISION, nullable=False)
    dec_error = Column(DOUBLE_PRECISION, nullable=False)
    parallax = Column(DOUBLE_PRECISION)
    parallax_error = Column(DOUBLE_PRECISION)
    pm = Column(REAL)
    pmra = Column(DOUBLE_PRECISION)
    pmra_error = Column(DOUBLE_PRECISION)
    pmdec = Column(DOUBLE_PRECISION)
    pmdec_error = Column(DOUBLE_PRECISION)
    astrometric_excess_noise = Column(DOUBLE_PRECISION, nullable=False)
    astrometric_excess_noise_sig = Column(DOUBLE_PRECISION, nullable=False)
    ruwe = Column(REAL)
    phot_g_n_obs = Column(Integer, nullable=False)
    phot_g_mean_flux_over_error = Column(REAL)
    phot_g_mean_mag = Column(REAL)
    phot_bp_n_obs = Column(Integer, nullable=False)
    phot_bp_mean_flux_over_error = Column(REAL)
    phot_bp_mean_mag = Column(REAL)
    phot_rp_n_obs = Column(Integer, nullable=False)
    phot_rp_mean_flux_over_error = Column(REAL)
    phot_rp_mean_mag = Column(REAL)
    phot_bp_rp_excess_factor = Column(REAL)
    phot_proc_mode = Column(Integer)
    bp_rp = Column(REAL)
    bp_g = Column(REAL)
    g_rp = Column(REAL)
    radial_velocity = Column(DOUBLE_PRECISION)
    radial_velocity_error = Column(DOUBLE_PRECISION)
    rv_method_used = Column(Integer)
    phot_variable_flag = Column(String, nullable=False)
    in_qso_candidates = Column(Boolean, nullable=False)
    in_galaxy_candidates = Column(Boolean, nullable=False)
    non_single_star = Column(Integer, nullable=False)
    has_epoch_photometry = Column(Boolean, nullable=False)
    classprob_dsc_combmod_quasar = Column(REAL)
    classprob_dsc_combmod_galaxy = Column(REAL)
    classprob_dsc_combmod_star = Column(REAL)
    teff_gspphot = Column(REAL)
    teff_gspphot_lower = Column(REAL)
    teff_gspphot_upper = Column(REAL)
    logg_gspphot = Column(REAL)
    logg_gspphot_lower = Column(REAL)
    logg_gspphot_upper = Column(REAL)
    mh_gspphot = Column(REAL)
    mh_gspphot_lower = Column(REAL)
    mh_gspphot_upper = Column(REAL)
    distance_gspphot = Column(REAL)
    distance_gspphot_lower = Column(REAL)
