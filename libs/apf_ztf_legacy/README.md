> **FROZEN — legacy ZTF pipeline only.**
> This is a verbatim snapshot of `apf` from `main` (commit `b58b866b4`), with
> `confluent-kafka` bumped to `>=2.12.0,<2.13.0` for quimal. It exists so the
> legacy ZTF steps (which use the pre-refactor apf API: `DefaultPrometheusMetrics`,
> `prometheus_metrics=` constructor arg, etc.) can be built from source on the
> `multisurvey` branch, whose `libs/apf` is the incompatible refactored framework.
> Do **not** add features here. New pipeline code uses `libs/apf`. Retire when the
> legacy ZTF pipeline is decommissioned (~Jan 2026).
