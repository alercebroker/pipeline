...
def evaluate_all_models(results_folder_path, dataset_to_evaluate):
    # get acronims
    ...
    for acronym in folder_acronyms:
        ...
        preprocessed_data_array = model.preprocess_data(dataset_to_evaluate.data_array)
        ...
        for folder_name in folder_names_that_match_acronym:
            ...
            predictions = model.predict(preprocessed_data_array)
            ...
            start = time.time()
            preprocessed_sample = model.preprocess_data(sample)
            model.predict(preprocessed_sample)
            end = time.time()
...
if __name__ == "__main__":
    ...
    params = {
        ...
        param_keys.ENTROPY_REG_BETA: 0,
        param_keys.LEARNING_RATE: 0,
        param_keys.DROP_RATE: 0,
        param_keys.BATCH_SIZE: 16,
        param_keys.KERNEL_SIZE: 3,
    }
    aux_model = DeepHiTSEntropyRegModel(params)
    aux_model.data_loader.dataset_preprocessor.set_pipeline(
        [
            aux_model.data_loader.dataset_preprocessor.image_check_single_image,
            aux_model.data_loader.dataset_preprocessor.image_clean_misshaped,
            aux_model.data_loader.dataset_preprocessor.image_select_channels,
        ]
    )
    ...
    save_path = os.path.join(results_folder_path, "test_set_results.pkl")
