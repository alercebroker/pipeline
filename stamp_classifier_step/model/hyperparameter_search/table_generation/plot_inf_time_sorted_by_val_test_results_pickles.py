...
if __name__ == "__main__":
    color_random_state = np.random.randint(10000)  # 960#
    ...
    results_path = os.path.join(results_folder_path, "%s_set_results_1.pkl" % set_name)
    test_results = pd.read_pickle(results_path)

    val_results_path = os.path.join(
        results_folder_path, "%s_set_results_1.pkl" % val_set_name
    )
    ...
    # print('N models % i' % len(test_names))
    # print(test_names)
    colors = np.random.RandomState(color_random_state).rand(len(test_means), 3).tolist()
    best_5_models_colors = np.array(["r", "c", "b", "y", "m"])
    print(best_5_models_colors)
    ...
    for i in range(n_model_to_show):
        ...
        colors[test_idx_with_same_name_as_val] = best_5_models_colors[i]
    ...
    # COMMENT TO SORT BY TEST
    # sorted_indxs = np.argsort(test_n_model_means)[::-1]
    ...
    for i in range(n_model_to_show):
        ...
        # model_name_str = r"$M_{%i}$: " % i + model_name_str
    ...
    fig = plt.figure(figsize=(6.4, 4.8))
    test_n_models_idexes = []
    for i in range(len(means)):
        if names[i] in test_n_model_names:
            test_n_models_idexes.append(i)
            plt.scatter(
                time_means[i],
                means[i],
                marker="D",
