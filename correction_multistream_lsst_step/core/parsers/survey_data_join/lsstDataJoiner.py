    ...
    def post_process_data(self, combined_data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for key in ['sources', 'previous_sources', 'forced_sources']:
            ...
            if not df.empty:
                ...
                df = df.sort_values(["new", "mjd"], ascending=[False])
        ...
        result['ss_objects'] = combined_data.get('ss_objects', pd.DataFrame())
        result['dia_objects'] = combined_data.get('dia_objects', pd.DataFrame())
