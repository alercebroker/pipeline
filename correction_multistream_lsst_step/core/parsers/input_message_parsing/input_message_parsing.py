...
def get_input_message_parser(survey: str) -> InputMessageParsingStrategy:
    ...
    Example:
        >>> parser = get_input_message_parser('lsst')
        >>> parsed_data = parser.parse_input_messages(raw_messages)
        >>> schemas = parser.get_input_schema_config()
    
    Note:
        The dynamic imports prevent loading unused parsers, which is important
        because each parser imports heavy dependencies like survey-specific schemas.
    ...
    if survey_lower == 'lsst':
        ...
        raise ValueError(
            ...
            f"Survey names are case-insensitive."
        )
