...
def conv(np.ndarray[float, ndim=1, mode="c"] a,
         ...
         value = _conv(N,<float *> &a[0],<float *> &time1[0],<float *> &b1[0],<float *> &b2[0],<float *> &mask[0],c1,c2,d,dtmin)
         return value,b1,b2
...
def _statistics32(np.ndarray[float, ndim=1, mode="c"] mag,
  ...
  _statistics(N,<float*> &flux[0],<float*> &mag[0],<float*> &magerr[0], <float*> &time[0], <float*> &mask[0],
