...
base_schema_path = Path("../schemas/surveys/lsst_v8.0/")
alert_schema = load_schema(str(base_schema_path / "lsst.v8_0.alert.avsc"))
diaObject_schema = load_schema(str(base_schema_path / "lsst.v8_0.diaObject.avsc"))
...
def generate_objects_dia_only(alert: Message) -> Message:
...
def generate_alerts(n: int = 30) -> Iterable[Message]:
    ...
    alerts = map(lambda alert: generate_objects_dia_only(alert), alerts)
