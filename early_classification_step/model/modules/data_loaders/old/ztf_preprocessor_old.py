...
class ZTFDataPreprocessorOld(object):
    """
    Constructor
    """
  ...
  """
    ...
    def preprocesses_dataset_old(self, dataset_obj: Dataset):
        dataset_no_misshaped = self.clean_misshaped(dataset_obj)
        dataset_no_nans = self.clean_nans(dataset_no_misshaped)
        # cropped_at_center_samples = self.crop_at_center(
        #     numpy_misshape_clean_samples)
        ...
        preprocessed_dataset = dataset_no_nans
        preprocessed_dataset = Dataset(
            np.stack(preprocessed_dataset.data_array),
            np.array(preprocessed_dataset.data_label),
            dataset_obj.batch_size,
        )
        return preprocessed_dataset

    def _select_channels(self, data_array, channels_to_select):
        if len(data_array.shape) == 1:
            raise ValueError(
                "Data array has shape %s, It's probabily due to misshapes in np.array(dataset). Try cleaning misshapes first"
            )
        selected_images_channels = data_array[..., channels_to_select]
        if len(selected_images_channels.shape) == 3:
            selected_images_channels = selected_images_channels[..., np.newaxis]
        return selected_images_channels

    def normalize_by_sample(self, images):
    ...
    def preprocesses_dataset_clean_misshaped_nans(
        self, dataset_obj: Dataset, channels_to_select
    ):
        dataset_obj.data_array = self._select_channels(
            dataset_obj.data_array, channels_to_select
        )
        dataset_no_misshaped = self.clean_misshaped(dataset_obj)
        dataset_no_nans = self.clean_nans(dataset_no_misshaped)
        dataset_no_nans.data_array = self.normalize_by_sample(
            dataset_no_nans.data_array
        )
        preprocessed_dataset = dataset_no_nans
        preprocessed_dataset = Dataset(
            np.stack(preprocessed_dataset.data_array),
    ...
    def nan_to_num(self, samples, number_to_replace_nans):
    ...
    def preprocesses_dataset_clean_misshaped(
        self, dataset_obj: Dataset, channels_to_select
    ):
        dataset_obj.data_array = self._select_channels(
            dataset_obj.data_array, channels_to_select
        ...
        dataset_no_misshaped = self.clean_misshaped(dataset_obj)
        dataset_no_misshaped.data_array = self.normalize_by_sample(
            dataset_no_misshaped.data_array
        )
        dataset_no_misshaped.data_array = self.nan_to_num(
            dataset_no_misshaped.data_array
        )
        preprocessed_dataset = dataset_no_misshaped
        preprocessed_dataset = Dataset(
            np.stack(preprocessed_dataset.data_array),
            np.array(preprocessed_dataset.data_label),
            dataset_obj.batch_size,
    ...
    def get_misshaped_samples_idx(self, samples):
    ...
    def clean_misshaped(self, dataset_obj: Dataset):
        samples_clone = list(dataset_obj.data_array[:])
        labels_clone = list(dataset_obj.data_label[:])
        miss_shaped_sample_idx = self.get_misshaped_samples_idx(samples_clone)
        ...
        return Dataset(samples_clone, labels_clone, dataset_obj.batch_size)

    def get_nans_samples_idx(self, samples):
        nans_sample_idx = []
        for i in range(len(samples)):
            sample = samples[i]
    ...
    def clean_nans(self, dataset_obj: Dataset):
        samples_clone = list(dataset_obj.data_array[:])
        labels_clone = list(dataset_obj.data_label)
        nans_sample_idx = self.get_nans_samples_idx(samples_clone)
