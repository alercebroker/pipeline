...
class AlerceApi(object):
    ...
    def query(self, params):
        # show api results
        r = self.session.post(url="%s/query" % self.baseurl, json=params)
    ...
    def get_sql(self, params):
        r = self.session.post(url="%s/get_sql" % self.baseurl, json=params)
        return r.content

    def get_detections(self, params):
        # show api results
        r = self.session.post(url="%s/get_detections" % self.baseurl, json=params)
        df = pd.DataFrame(r.json())
    ...
    def get_non_detections(self, params):
        # show api results
        r = self.session.post(url="%s/get_non_detections" % self.baseurl, json=params)
        df = pd.DataFrame(r.json())
    ...
    def get_stats(self, params):
        # show api results
        r = self.session.post(url="%s/get_stats" % self.baseurl, json=params)
        df = pd.DataFrame(r.json())
    ...
    def get_probabilities(self, params):
        # show api results
        r = self.session.post(url="%s/get_probabilities" % self.baseurl, json=params)
    ...
    def get_features(self, params):
        # show api results
        r = self.session.post(url="%s/get_features" % self.baseurl, json=params)
...
class AlerceStamps(object):
    ...
    def _get_candid(self, object_ids):
        ...
        for ob_id in tqdm(object_ids, total=len(object_ids), disable=DISABLE_TQDM):
            params = {"oid": ob_id}
    ...
    def download_avros(
        ...
        if return_alert_series:
            return alert_series, paths
