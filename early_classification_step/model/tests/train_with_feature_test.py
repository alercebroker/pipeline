...
if __name__ == "__main__":
    ...
    # train, val, test = model.data_loader.get_raw_datasets_splitted().values()
    # val = model.dataset_preprocessor.preprocess_dataset(val)
    # model.evaluate(val.data_array, val.meta_data, val.data_label)
    # train, val, test = model.data_loader.get_raw_datasets_splitted().values()
    # model.close()
    # del model
    # aux_model = DeepHiTSWithFeaturesEntropyReg(params, model_name)
    # aux_model.load_model(model_path)
    # val = aux_model.dataset_preprocessor.preprocess_dataset(val)
