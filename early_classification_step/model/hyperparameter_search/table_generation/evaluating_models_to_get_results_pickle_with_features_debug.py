...
def evaluate_all_models(results_folder_path, dataset_to_evaluate, params):
    # get acronims
    ...
    for acronym in tqdm(folder_acronyms):
        ...
        for folder_name in folder_names_that_match_acronym:
            ...
            # weights_path = os.path.join(
            #     #PROJECT_PATH, 'results', 'hyperparameter_search_17_05_20',
            #     results_folder_path,
...
if __name__ == "__main__":
    ...
    params = aux_model.params.copy()
    aux_data_loader = FrameToInputWithFeatures(params)
    dataset_dicts = aux_data_loader.get_raw_datasets_splitted()
    test_set = dataset_dicts[general_keys.TEST]
    aux_model.close()
    del aux_data_loader
    del aux_model
    del dataset_dicts
    ...
    results_dict, execution_times_dict = evaluate_all_models(
        results_folder_path, test_set, params
    )
