...
from reflector_step.utils import producer
...
class TestRawProducer(TestCase):
    def test_serialization_only_returns_message_value(self, mock_producer):
        producer = RawKafkaProducer({"TOPIC": None, "PARAMS": {}, "SCHEMA_PATH": PATH})
    ...
    def test_initialization_with_schema_logs_warning(self, mock_producer):
        with self.assertLogs(logger="alerce.RawKafkaProducer", level="WARNING"):
            RawKafkaProducer(
                {
                    "SCHEMA": {
                        "name": "dummy",
...
class TestStep(TestCase):
    def setUp(self):
        ...
        self.mock_producer = mock.create_autospec(GenericProducer)
