...
def multi_band(time, flux, band, preferred):
    for fid in preferred:
        ...
        raise ValueError(
            f"None of bands {preferred} found in the provided bands"
        )
    ...
    for fid in np.unique(band):
        ...
        ampl_guess = np.clip(
            1.2 * fmax, ampl_bounds[0] * 1.1, ampl_bounds[1] * 0.9
        )

        bounds += [ampl_bounds] + static_bounds
        guess += [ampl_guess] + static_guess

    return np.array(guess, dtype=np.float32), np.array(
...
def single_band(time, flux, alt=False, bug=False):
    ...
    if alt:
        ...
        bounds = [
            [fmax / 3, -50, 1, 0, 1, 1],
            [fmax * 3, 50, 100, 1, 100, 100],
        ]
        ampl = 3 * fmax if bug else 1.2 * fmax
        guess = np.clip(
            [ampl, -5, np.max(time), 0.5, time[imax] / 2, 40], *bounds
        )
