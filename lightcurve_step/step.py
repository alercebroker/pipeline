...
class LightcurveStep(GenericStep):
    ...
    def unique_non_detections(old_non_detections, new_non_detections):
        ...
        new_non_detections = [det for det in new_non_detections if create_id(det) not in ids]
    ...
    def pre_execute(self, messages: list[dict]):
        ...
        for message in messages:
            if message["aid"] in aids:
                ...
                output[idx]["detections"] = self.unique_detections(output[idx]["detections"], message["detections"])
                output[idx]["non_detections"] = self.unique_non_detections(output[idx]["non_detections"], message["non_detections"])
            else:
    ...
    def execute(self, messages: list[dict]):
        for message in messages:
            ...
            message["non_detections"] = self.unique_non_detections(non_detections_in_db, message["non_detections"])
